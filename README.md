...
- KLPBBS 推广任务参考 `klpAutomation`，从全部来源收集并全局去重最多 2000 个代理，每批最多用 20 个 worker 并行点击站点校验过的同源推广链接；代理失败时继续下一批，HTTP 成功不等于任务进度。每批后由主线程读取任务页 `#csc_1` 的实际百分比，直到完成领奖或代理池耗尽。只有推广点击绕过 WARP 路由选择，代理源下载、任务申请、状态检查和领奖仍走 WARP。
