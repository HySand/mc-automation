...
`item=doing` confirmation and reports success only when task ID 1's `do=draw` link is absent;
navigation or `do=apply` links with the same task ID are not treated as an incomplete draw.
